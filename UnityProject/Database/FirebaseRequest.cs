...
namespace UnityProject.Database
{
    using System;
    ...
    class FirebaseRequest
    {
        ...
        {
            Uri requestURI;
            if (Helper.ValidateURI(this.Uri))
            {
            ...
            if (this.JSON != null)
            {
                if (!Helper.TryParseJSON(this.JSON, out json))
                {
                    return new FirebaseResponse(false, string.Format("Invalid JSON : {0}", json));
                }
            }

            var response = Helper.RequestHelper(this.Method, requestURI, json);
            response.Wait();
